...
class BaseModel extends Model{
    ...
    +----------------------------------------------------------
     */
    public static function getPageList($param = array('modelName' => '', 'field' => '*', 'order' => 'id DESC', 'listRows' => '10', 'tablePrefix' => '', 'connection' => '', 'modelType' => 'commonModel', 'relationModel' => ''), $condition = '') {
        ...
        // 生成分页session数据
        if($data['pageCurrent']==""||$data['pageCurrent']==null){
            $data['pageCurrent']=0;
        }
        ...
        //查看该模型的sql语句
//        echo $sqlStr = $M->getlastsql()."  hello".$data['pageCurrent'];exit();
        return $list;
    ...
    public static function getCommonPageList($param=array('modelName'=>'','field'=>'*','order' => 'id DESC', 'listRows' => '20', 'tablePrefix' => '', 'connection' => '', 'modelType' => 'commonModel', 'relationModel' => ''), $condition = '') {
        ...
        $Page=new Page($count,$param['listRows']);
...
//        echo $sqlStr = $M->getlastsql();
    ...
    +----------------------------------------------------------
     */
    public static function getDetailNoCache($param = array('modelName' => '', 'field' => '*', 'tablePrefix' => '', 'connection' => '', 'modelType' => '', 'relationModel' => ''), $condition) {
        $M = self::getModel($param);
        if($param['modelType'] == 'relationModel'){
            $relationModel = isset($param['relationModel']) ? $param['relationModel'] : true;
            $info = $M->relation($relationModel)->where($condition)->find();
...
//            $result = $M->relation($relationModel)->add($data);
    ...
    /**
    +----------------------------------------------------------
     *bjui doajax小窗口对应的 写入数据表操作
     * @return  array  返回写入状态
    +----------------------------------------------------------
     */
    public static function insert2($param = array('modelName' => '', 'returnUrl' => '', 'tablePrefix' => '', 'connection' => '', 'modelType' => '', 'relationModel' => ''), $data) {
        $M = self::getModel($param);
        $data['cUid'] = self::getMyInfo('uid');
        $data['cTime'] = $data['uTime'] = time();
        ...
        if ($result) {
            self::OperationLogInsert($sqlStr);
            return array('statusCode' => 200, 'message' => C('ALERT_MSG.EXECUTE_SUCCESS'), 'closeCurrent' => false);
        } else {
            return array('statusCode' => 300, 'message' => C('ALERT_MSG.EXECUTE_FAILED'));
        }
    }
